...
# certabo helpers
from lichess.certabo import codes
from lichess.certabo import serialreader, btserialreader
...
class Certabo:
    def __init__(self, port, connection_method, kill_thread_event, calibrate=0, **kwargs):
        ...
        if connection_method == 'bluetooth':
            self.serialthread = btserialreader.serialreader(self.handle_usb_data, self.portname, kill_thread_event)
        else:
    ...
    def calibrate_from_usb_data(self, usb_data):
        ...
        if self.calibration_samples_counter >= 15:
            ...
            codes.calibration(usb_data, self.new_setup, self.calibration_data_path)
            self.calibration = False
