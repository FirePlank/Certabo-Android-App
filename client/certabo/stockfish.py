...
from cfg import DEBUG_PYSTOCKFISH
...
class EngineThread(threading.Thread):
    ...
    def run(self):
        ...
        self.engine = Engine(depth=self.difficulty, binary=self.engine_path, param=self.engine_parameters, chess960=self.chess960)
        if DEBUG_PYSTOCKFISH:
            logging.debug("Setting position to %s", self.move_history)
